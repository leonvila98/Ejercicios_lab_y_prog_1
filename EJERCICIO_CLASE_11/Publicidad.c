...
int publicidadMenorADias(Publicidad *pPublicidad,Pantalla *pPantalla,int dias,int limite)
{
    int i;
    float precioTotal;
    int indicePantalla;
    ...
    for(i=0;i<limite;i++)
    {
        ...
        {
            if(pPublicidad[i].dias<=dias)
            {
                idPantalla=pPublicidad[i].idPantalla;
                indicePantalla=pan_findPanById(pPantalla,limite,idPantalla);
                precioTotal=pPublicidad[i].dias*pPantalla[indicePantalla].precio;
                pan_printUnaPantalla(pPantalla,idPantalla,limite);
                printf("\nDias:%d Contratacion de:%s Precio total:%.2f\n",  pPublicidad[i].dias,
                                                                            pPublicidad[i].cuit,
                                                                            precioTotal);
