...
int burbujeoStructFloat(Pantalla *pPantalla,int limite)
{
    int retorno=-1;
    int i;
    int k;
        ...
        for(i=0;i<limite;i++)
        {
            ...
            else if(pPantalla[i].precio==pPantalla[i+1].precio)
            {
                do
                {
                    for(k=0;k<limite;k++)
                    {
                        if(strncmp(pPantalla[k].nombre,pPantalla[k+1].nombre,50)>0)
                        {
                            buffer=pPantalla[k];
                            pPantalla[k]=pPantalla[k+1];
                            pPantalla[k+1]=buffer;
                            retorno=1;
                        }
                    }
                }while(flag==0);
...
int pan_printUnaPantalla(Pantalla* pPantalla,int id,int len)
{
    int i;
    int flag=1;
    for(i=0;i<len;i++)
    {
        if(pPantalla[i].isEmpty==0 && pPantalla[i].idPantalla==id)
        {
            printf("\nID: %d\nNombre: %s\nDireccion: %s\nPrecio: %.2f\nTipo: %d\n-------",
                   pPantalla[i].idPantalla,pPantalla[i].nombre,pPantalla[i].direccion,pPantalla[i].precio,pPantalla[i].tipo);
            flag=0;
