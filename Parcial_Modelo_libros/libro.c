...
int libro_buscarString(Libro array[], int size, char* valorBuscado, int* indice)                    //cambiar libro
{
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(strcmp(array[i].nombreLibro,valorBuscado)==0)                                        //cambiar campo nombreLibro
            {
                *indice=i;
                retorno=0;
                break;
...
int libro_baja(Libro array[], int sizeArray)                                      //cambiar libro
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            strcpy(array[posicion].nombreLibro,"");                                                   //cambiar campo nombreLibro
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && sizeArray>0)
    {
        for(i=0;i<sizeArray;i++)
        {
            if(array[i].idUnico==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                ...
                strcpy(array[i].nombreLibro,"");                                                   //cambiar campo nombreLibro
            }
        }
        retorno=0;
    }
...
int libro_ordenarPorDobleCriterio(Libro array[],int size, int orderFirst, int orderSecond)                              //cambiar libro
{
    ...
    if(array!=NULL && size>=0)
    {
        do
        {
            ...
            for (i = 1; i < size-1; i++)
            {
                if( ((strcmp(array[i].nombreLibro,array[i+1].nombreLibro) < 0) && orderFirst) ||
                    ((strcmp(array[i].nombreLibro,array[i+1].nombreLibro) > 0) && !orderFirst) )
                {
                    flagSwap=1;
                    buffer = array[i];
                    array[i] = array[i+1];
                ...
                /*else if(strcmp(array[i].nombreLibro,array[i+1].nombreLibro) == 0)
                {
...
int libro_listar(Libro array[], int size)                      //cambiar libro
{
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            ...
            {
                printf("\n Posicion: %d\n ID: %d\n idAutor: %d\n nombreLibro: %s",
                        i,array[i].idUnico,array[i].idAutor,array[i].nombreLibro);
            }
        }
        retorno=0;
    }
